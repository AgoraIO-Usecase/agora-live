...
open class AUINetworking: NSObject {
    ...
    private func baserequest(model: AUINetworkModel, progress: ((Float) -> Void)?, completion: ((Error?, Any?) -> Void)?) {
        ...
        let handleResponse: ((Data?,URLResponse?,Error?) ->Void) = { data, response, error in
            ...
            if let error = error {
                agoraEnt_error("request fail: \(error), curl: \(urlStr)", tag: "AUINetworking")
            ...
            guard let data = data else {
                agoraEnt_error("parse fail: data empty,curl: \(urlStr)", tag: "AUINetworking")
                DispatchQueue.main.async {
            ...
            do {
                ...
                agoraEnt_error("parse fail throw: , curl: \(urlStr) error: \(maskSensitiveParameters(err.localizedDescription))", tag: "AUINetworking")
                agoraEnt_error("parse fail: \(maskSensitiveParameters(String(data: data, encoding: .utf8) ?? "nil")),curl: \(urlStr)", tag: "AUINetworking")
                DispatchQueue.main.async {
                    completion?(err, nil)
                }
                return
            ...
            agoraEnt_info("request success url = \(urlStr) message = \(maskSensitiveParameters(String(data: data, encoding: .utf8) ?? "nil"))", tag: "AUINetworking")
            DispatchQueue.main.async {
                completion?(nil, obj)
            }
        }
