...
- (void)setMakeup: (BOOL)isSelected {
    ...
    if (isSelected) {
        ...
        NSString *makeupPath = [bundle pathForResource:@"makeup/ziyun" ofType:@"bundle"];
...
- (void)setStickerPath: (NSString *)stickerName {
    NSBundle *bundle = [BundleUtil bundleWithBundleName:@"FURenderKit" podName:@"fuLib"];
    NSString *path = [bundle pathForResource:[NSString stringWithFormat:@"sticker/%@", stickerName] ofType:@"bundle"];
    if (!path) {
