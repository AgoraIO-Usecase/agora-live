...
class VRSoundEffectsViewController: VRBaseViewController {
    ...
    lazy var toLive: UIButton = {
        let button = UIButton(type: .custom).frame(CGRect(x: 30, y: 15, width: ScreenWidth - 60, height: 50)).title("voice_go_live".voice_localized, .normal).font(.systemFont(ofSize: 16, weight: .semibold)).setGradient([UIColor(red: 0.13, green: 0.608, blue: 1, alpha: 1), UIColor(red: 0.204, green: 0.366, blue: 1, alpha: 1)], [CGPoint(x: 0, y: 0), CGPoint(x: 0, y: 1)]).cornerRadius(25).addTargetFor(self, action: #selector(VRSoundEffectsViewController.goLive), for: .touchUpInside)
        button.titleLabel?.accessibilityIdentifier = "voice_chat_sound_effect_go_live_btn"
        return button
    }()
    ...
    override func viewDidLoad() {
        ...
        navigation.title.text = "voice_sound_selection".voice_localized
