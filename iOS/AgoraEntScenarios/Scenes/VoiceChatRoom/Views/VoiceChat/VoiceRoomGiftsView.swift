...
public class VoiceRoomGiftsView: UIView, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    public var sendClosure: ((VoiceRoomGiftEntity) -> Void)?

    var lastPoint = CGPoint.zero

    lazy var header: VoiceRoomAlertContainer = .init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 60)).backgroundColor(.white)
    ...
    lazy var giftList: UICollectionView = .init(frame: CGRect(x: 15, y: self.header.frame.maxY, width: ScreenWidth - 30, height: (110 / 84.0) * ((ScreenWidth - 30) / 4.0)), collectionViewLayout: self.flowLayout).registerCell(VoiceRoomSendGiftCell.self, forCellReuseIdentifier: "VoiceRoomSendGiftCell").delegate(self).dataSource(self).showsHorizontalScrollIndicator(false).backgroundColor(.white)
    ...
    lazy var contribution: UILabel = .init(frame: CGRect(x: 20, y: self.giftList.frame.maxY + 50, width: ScreenWidth / 2.0 - 40, height: 20)).font(.systemFont(ofSize: 12, weight: .regular)).textColor(UIColor(0x6C7192)).text("\(VoiceRoomUserInfo.shared.user?.amount ?? 0)")
    ...
    lazy var send: UIButton = {
        UIButton(type: .custom).frame(CGRect(x: self.chooseQuantity.frame.maxX, y: 0, width: 79, height: 40)).font(.systemFont(ofSize: 14, weight: .semibold)).setGradient([UIColor(0x219BFF), UIColor(0x345DFF)], [CGPoint(x: 0, y: 0), CGPoint(x: 0, y: 1)]).textColor(.white, .normal).title("voice_send".voice_localized, .normal).addTargetFor(self, action: #selector(sendAction), for: .touchUpInside)
    }()

    lazy var title: UILabel = .init(frame: CGRect(x: ScreenWidth / 2.0 - 30, y: 25.5, width: 60, height: 20)).textAlignment(.center).textColor(UIColor(0x040925)).font(.systemFont(ofSize: 16, weight: .semibold)).text("voice_gifts".voice_localized)
    ...
    lazy var popview: VoiceRoomPopView = .init(frame: CGRect(x: self.lineLayer.frame.minX - 20, y: 25, width: 120, height: 186)).contentMode(.scaleAspectFill)
...
public extension VoiceRoomGiftsView {
    ...
    @objc internal func sendAction() {
        ...
        if sendClosure != nil, current != nil {
            ...
            sendClosure!(current!.mutableCopy() as! VoiceRoomGiftEntity)
        }
