...
public final class VRRoomListCell: UITableViewCell {
    ...
    lazy var entryRoom: UIButton = .init(type: .custom).frame(CGRect(x: self.background.frame.width - 72, y: self.background.frame.height - 40, width: 56, height: 24)).font(UIFont.systemFont(ofSize: 12, weight: .semibold)).backgroundColor(.clear).title("voice_enter".voice_localized, .normal)
    ...
    func setupViewsAttributes(room: VRRoomEntity) {
        ...
        if room.is_private == true {
            accessSymbol.set(image: UIImage.sceneImage(name: "suo", bundleName: "VoiceChatRoomResource"), title: "voice_private".voice_localized, titlePosition: .right, additionalSpacing: 5, state: .normal)
        ...
        if let portrait = room.owner?.portrait, portrait.hasPrefix("http") {
            ...
            ownerName.setImage(UIImage(named: room.owner?.portrait ?? ""), for: .normal)
        }
