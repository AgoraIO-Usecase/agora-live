...
public class VRCreateRoomInputView: UIView, UITextFieldDelegate {
    ...
    private let codeMessage = "voice_enter_4_digit_password".voice_localized

    private let nameMessage = "voice_please_set_a_name".voice_localized
    ...
    lazy var roomNameField: UITextField = .init(frame: CGRect(x: 45, y: self.roomName.frame.maxY + 15, width: ScreenWidth - 90, height: 40)).placeholder("voice_set_room_name".voice_localized).font(.systemFont(ofSize: 18, weight: .regular)).textColor(.darkText).delegate(self)

    lazy var roomEncryption: UILabel = .init(frame: CGRect(x: self.roomName.frame.minX, y: self.roomBackground.frame.maxY + 12, width: 60, height: 20)).font(.systemFont(ofSize: 14, weight: .regular)).textColor(.darkText).text("voice_room_access".voice_localized).backgroundColor(.clear)
...
public extension VRCreateRoomInputView {
    private func setupAttributes() {
        pinCode.alpha = 0
        randomName.set(image:UIImage.sceneImage(name: "random", bundleName: "VoiceChatRoomResource"), title: "voice_random".voice_localized, titlePosition: .right, additionalSpacing: 5, state: .normal)
