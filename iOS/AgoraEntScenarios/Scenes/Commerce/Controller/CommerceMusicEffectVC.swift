...
class CommerceMusicEffectVC: UIViewController {
    
    var musicManager: CommerceMusicPresenter?
    
    var currentChannelId: String?
    
    private let headerView = CommerceMusicTableHeaderView()
    
    private lazy var tableView: UITableView = {
        let tableView = UITableView()
        tableView.backgroundColor = .commerce_dark_cover_bg
    ...
    deinit {
        commerceLogger.info("deinit-- ShowMusicEffectVC ")
    }
...
extension CommerceMusicEffectVC {
...
extension CommerceMusicEffectVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cellID = "CommerceMusicEffectCell"
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? CommerceMusicEffectCell
        if cell == nil {
            cell = CommerceMusicEffectCell(style: .default, reuseIdentifier: cellID)
        }
        ...
        cell?.setTitle(data.title, dataArray: data.dataArray, selectAction: {[weak self] index in
            ...
            if let channelId = self?.currentChannelId,
               let playerId = CommerceAgoraKitManager.shared.mediaPlayer()?.getMediaPlayerId() {
                let options = AgoraRtcChannelMediaOptions()
                options.publishMediaPlayerAudioTrack = true
                options.publishMediaPlayerId = Int(playerId)
                CommerceAgoraKitManager.shared.updateChannelEx(channelId: channelId, options: options)
            }
