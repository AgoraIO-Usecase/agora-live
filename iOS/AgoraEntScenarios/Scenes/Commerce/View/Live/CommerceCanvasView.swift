...
protocol CommerceCanvasViewDelegate: NSObjectProtocol {
...
//        view.addTarget(self, action: #selector(onTapRemoteButton), for: .touchUpInside)
    ...
    private lazy var localUser: AGEButton = {
        ...
        button.setImage(UIImage.commerce_sceneImage(name: "show_mic"),
                        for: .normal,
                        postion: .right,
                        spacing: 5)
        button.setImage(UIImage.commerce_sceneImage(name: "show_mic_off"),
                        for: .selected,
                        postion: .right,
                        spacing: 5)
        return button
    ...
    private lazy var timerView: UIButton = {
        let button = UIButton()
        button.setBackgroundImage(UIImage.commerce_sceneImage(name: "show_pk_timer_bg"), for: .normal)
    ...
    private lazy var pkView: UIImageView = {
        let imageView = UIImageView(image: UIImage.commerce_sceneImage(name: "show_pk_progress_bg"))
        return imageView
    }()
    ...
    var canvasType: CommerceLiveCanvasType = .none {
        didSet {
            switch canvasType {
            case .none:
