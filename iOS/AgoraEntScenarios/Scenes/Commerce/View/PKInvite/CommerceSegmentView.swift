...
public enum CommerceSegmentIndicatorStyle {
    case line, cover
}

public struct CommerceSegmentViewStyle {
    public var indicatorStyle: CommerceSegmentIndicatorStyle = .cover
...
@IBDesignable public class CommerceSegmentView: UIControl {

    public struct CommerceTitleElement: Equatable {
        ...
        public static func == (lhs: CommerceTitleElement, rhs: CommerceTitleElement) -> Bool {
            return lhs.title == rhs.title && lhs.selectedImage == rhs.selectedImage && lhs.normalImage == rhs.selectedImage
        }
    }

    public var style: CommerceSegmentViewStyle {
        didSet {
            reloadLayout()
        }
    }
    ...
    private var _titleElements: [CommerceTitleElement] {
        didSet {
            reloadData(animated: false, sendAction: false)
        }
    }
    public var titleElements: [CommerceTitleElement] {
        get {
    ...
    @IBInspectable public var titles: [String] {
        ...
        set {
            _titleElements = newValue.map({ CommerceTitleElement(title: $0) })
        }
    }
    ...
    public convenience override init(frame: CGRect) {
        self.init(frame: frame, segmentStyle: CommerceSegmentViewStyle(), titles: [])
    }

    public convenience init(frame: CGRect, titles: [String]) {
        self.init(frame: frame, segmentStyle: CommerceSegmentViewStyle(), titles: titles)
    }

    public init(frame: CGRect, segmentStyle: CommerceSegmentViewStyle, titles: [String]) {
        self.style = segmentStyle
        self._titleElements = titles.map({ CommerceTitleElement(title: $0)})
        super.init(frame: frame)
        shareInit()
    }

    public convenience init(frame: CGRect, segmentStyle: CommerceSegmentViewStyle, richTextTitles: [CommerceTitleElement]) {
        self.init(frame: frame, segmentStyle: segmentStyle, titles: [])
        setRichTextTitles(richTextTitles)
    }

    required public init?(coder aDecoder: NSCoder) {
        self.style = CommerceSegmentViewStyle()
        self._titleElements = []
    ...
    public func setRichTextTitles(_ titles: [CommerceTitleElement]) {
    ...
    private func reloadData(animated: Bool = true, sendAction: Bool = true) {
        ...
        for (index, item) in titleElements.enumerated() {
            ...
            if let normalImage = item.normalImage {
                backLabel.commerce_addToLeft(image: normalImage)
            }
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(CommerceSegmentView.handleTapGesture(_:)))
...
extension UILabel {
    @objc fileprivate func commerce_addToLeft(image: UIImage?) {
