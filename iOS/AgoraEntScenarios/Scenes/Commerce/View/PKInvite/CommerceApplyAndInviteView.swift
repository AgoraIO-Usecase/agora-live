...
enum CommerceApplyAndInviteType: String, CaseIterable {
    case apply
    case invite
    
    var title: String {
        switch self {
        case .apply: return "show_linking_request_message".commerce_localized
        case .invite: return "show_onseat_invite".commerce_localized
        }
    }
}

class CommerceApplyAndInviteView: UIView {
    var applyStatusClosure: ((CommerceInteractionStatus) -> Void)?
    
    private lazy var segmentView: CommerceSegmentView = {
        let segmentView = CommerceSegmentView(frame: CGRect(x: 10,
                                                        y: 23,
                                                        width: Screen.width,
                                                        height: 44),
                                          segmentStyle: .init(),
                                          titles: CommerceApplyAndInviteType.allCases.map({ $0.title }))
        ...
        segmentView.valueChange = { [weak self] index in
            ...
            if index == 0 {
                self?.tableView.emptyTitle = "show_empty_onseat_apply".commerce_localized
                self?.getApplyList()
            } else {
                self?.tableView.emptyTitle = "show_no_user_yet".commerce_localized
                self?.getInviteList()
            }
        }
        return segmentView
    ...
    private lazy var endButton: AGEButton = {
        ...
        button.setTitle("show_stop_pking".commerce_localized, for: .normal)
        button.setTitleColor(UIColor(hex: "#684BF2"), for: .normal)
        button.setImage(UIImage.commerce_sceneImage(name: "show_live_close"),
                        for: .normal,
                        postion: .right,
                        spacing: 5)
    ...
    private lazy var tableView: AGETableView = {
        ...
        view.rowHeight = 67
        view.emptyTitle = "show_empty_onseat_apply".commerce_localized
        view.emptyTitleColor = UIColor(hex: "#989DBA")
        view.emptyImage = UIImage.commerce_sceneImage(name: "show_pkInviteViewEmpty")
        view.delegate = self
        view.register(CommerceSeatApplyAndInviteViewCell.self,
                      forCellWithReuseIdentifier: CommerceSeatApplyAndInviteViewCell.description())
        return view
    }()
    private var tipsViewHeightCons: NSLayoutConstraint?
    private var roomId: String!
    private var type: CommerceApplyAndInviteType = .apply
    var seatMicModel: CommerceInteractionInfo? {
        didSet {
            if seatMicModel == oldValue {
                return
            }
            self.tipsContainerView.isHidden = seatMicModel == nil
            self.tipsLabel.text = String(format: "show_onseat_with_broadcastor".commerce_localized, seatMicModel?.userName ?? "")
    ...
    private func getApplyList() {
        AppContext.commerceServiceImp(roomId)?.getAllMicSeatApplyList {[weak self] _, list in
    ...
    private func getInviteList() {
        AppContext.commerceServiceImp(roomId)?.getAllUserList {[weak self] _, list in
    ...
    private func getApplyPKInfo() {
        AppContext.commerceServiceImp(roomId)?.getCurrentApplyUser(roomId: roomId) {[weak self] roomModel in
            self?.tipsContainerView.isHidden = roomModel == nil
            self?.tipsLabel.text = String(format: "show_pking_with_broadcastor".commerce_localized, roomModel?.ownerName ?? "")
        }
    }
    ...
    private func onTapEndButton(sender: AGEButton) {
        ...
        if let model = seatMicModel {
            AppContext.commerceServiceImp(roomId)?.stopInteraction(interaction: model) { _ in }
        }
        applyStatusClosure?(.idle)
    }
}
extension CommerceApplyAndInviteView: AGETableViewDelegate {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CommerceSeatApplyAndInviteViewCell.description(),
                                                 for: indexPath) as! CommerceSeatApplyAndInviteViewCell
        cell.roomId = roomId
