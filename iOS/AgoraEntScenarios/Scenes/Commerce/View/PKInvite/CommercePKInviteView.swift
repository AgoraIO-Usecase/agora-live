...
class CommercePKInviteView: UIView {
    var roomId: String!
    var pkUserInvitationList: [CommercePKUserInfo]? {
        didSet {
            tableView.dataArray = pkUserInvitationList ?? []
        }
    }
    var createPKInvitationMap: [String: CommercePKInvitation]? {
        didSet {
            tableView.reloadData()
        }
    }
    var interactionList: [CommerceInteractionInfo]? {
        didSet {
            ...
            pkTipsLabel.text = String(format: "show_pking_with_broadcastor".commerce_localized, pkInfo?.userName ?? "")
        }
    }
    private lazy var titleLabel: AGELabel = {
        let label = AGELabel(colorStyle: .black, fontStyle: .large)
        label.text = "show_pking_request_message".commerce_localized
        return label
    }()
    ...
    private lazy var endButton: AGEButton = {
        ...
        button.setImage(UIImage.commerce_sceneImage(name: "show_live_end"),
                        for: .normal,
                        postion: .right,
                        spacing: 5)
    ...
    private lazy var tableView: AGETableView = {
        ...
        view.rowHeight = 67
        view.emptyTitle = "show_empty_online_boardcaster".commerce_localized
        view.emptyTitleColor = UIColor(hex: "#989DBA")
        view.emptyImage = UIImage.commerce_sceneImage(name: "show_pkInviteViewEmpty")
        view.delegate = self
        view.register(CommercePKInviteViewCell.self,
                      forCellWithReuseIdentifier: CommercePKInviteViewCell.description())
        return view
    }()
    ...
    private func onTapEndButton(sender: AGEButton) {
        ...
        AppContext.commerceServiceImp(roomId)?.stopInteraction(interaction: pkInfo) { error in
        }
    }
...
extension CommercePKInviteView: AGETableViewDelegate {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CommercePKInviteViewCell.description(),
                                                 for: indexPath) as! CommercePKInviteViewCell
        cell.roomId = roomId
        cell.pkUser = pkUserInvitationList?[indexPath.row]
        cell.pkInvitation = createPKInvitationMap?[cell.pkUser?.roomId ?? ""]
        return cell
    }
}
