...
class CommerceApplyView: UIView {
    ...
    private lazy var tipsLabel: AGELabel = {
        ...
        let text = " "+"show_onseat_waitting".commerce_localized
        let attrs = NSMutableAttributedString(string: text)
        let attr = NSAttributedString(string: "0" + "show_user_count".commerce_localized, attributes: [.font: UIFont.systemFont(ofSize: 14, weight: .bold)])
    ...
    private lazy var revokeutton: AGEButton = {
        ...
        button.setTitle("show_cancel_linking".commerce_localized, for: .normal)
        button.setTitleColor(UIColor(hex: "#684BF2"), for: .normal)
        button.setImage(UIImage.commerce_sceneImage(name: "show_live_withdraw"),
                        for: .normal,
                        postion: .right,
                        spacing: 5)
    ...
    private lazy var tableView: AGETableView = {
        ...
        view.rowHeight = 67
        view.emptyTitle = "show_no_user_apply_onseat".commerce_localized
    ...
    var interactionModel: CommerceInteractionInfo? {
        didSet {
            self.revokeutton.setTitle("show_stop_pking".commerce_localized, for: .normal)
    ...
    func getAllMicSeatList(autoApply: Bool) {
        guard let imp = AppContext.commerceServiceImp(roomId) else {
            return
        }
        imp.getAllMicSeatApplyList {[weak self] _, list in
            ...
            if seatUserModel == nil, autoApply, self?.interactionModel?.userId != VLUserCenter.user.id {
                self?.revokeutton.setTitle("show_cancel_linking".commerce_localized, for: .normal)
                self?.revokeutton.setImage(UIImage.commerce_sceneImage(name: "show_live_withdraw"),
                                          for: .normal,
                                          postion: .right,
                                          spacing: 5)
    ...
    private func setupTipsInfo(count: Int) {
        let text = " "+"show_onseat_waitting".commerce_localized
        let attrs = NSMutableAttributedString(string: text)
        let attr = NSAttributedString(string: "\(count)"+"show_user_count".commerce_localized,
...
//            revokeutton.isHidden = true
            AppContext.commerceServiceImp(roomId)?.cancelMicSeatApply { _ in }
            let index = tableView.dataArray?.firstIndex(where: { ($0 as? CommerceMicSeatApply)?.userId == VLUserCenter.user.id }) ?? 0
            tableView.dataArray?.remove(at: index)
            setupTipsInfo(count: dataArray.count)
        } else if let interactionModel = interactionModel {
            AppContext.commerceServiceImp(roomId)?.stopInteraction(interaction: interactionModel) { _ in }
            AlertManager.hiddenView()
        }
    }
}

extension CommerceApplyView: AGETableViewDelegate {
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CommerceApplyViewCell.description(),
                                                 for: indexPath) as! CommerceApplyViewCell
        if let model = self.tableView.dataArray?[indexPath.row] as? CommerceMicSeatApply {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        titleLabel.text = "show_get_in_line".commerce_localized
...
class CommerceApplyViewCell: UITableViewCell {
    ...
    private lazy var avatarImageView: AGEImageView = {
        ...
        imageView.image = UIImage.commerce_sceneImage(name: "show_default_avatar")
    ...
    func setupApplyData(model: CommerceMicSeatApply, index: Int) {
        ...
        if model.userId == VLUserCenter.user.id {
            nameLabel.textColor = .commerce_zi01
        } else {
