...
class CommerceDebugSetting2TFCell: CommerceSettingBaseCell {
    ...
    func setTitle(_ title: String?, 
                  ...
                  value2: String?,
                  separator: String?,
                  tf1DidEndEditing: ((_ textField: UITextField)->())?,
                  tf2DidEndEditing: ((_ textField: UITextField)->())?,
                  beginEditing: (()->())?) {
...
extension CommerceDebugSetting2TFCell: UITextFieldDelegate {
    private func createTF() -> UITextField {
        let tf = UITextField()
        tf.borderStyle = .roundedRect
        tf.font = .commerce_R_14
        tf.textColor = .commerce_Ellipse5
        tf.delegate = self
