...
enum CommerceError: Int, Error {
    ...
    func desc() -> String {
        ...
        case .pkInteractionMaximumReach:
            return "show_error_pk_interaction_exist".commerce_localized
        case .seatInteractionMaximumReach:
            return "show_error_seat_interaction_exist".commerce_localized
        case .userCannotAccept:
            return "show_error_interaction_rejected_by_owner".commerce_localized
        default:
...
class CommerceSyncManagerServiceImp: NSObject, CommerceServiceProtocol {
    ...
    private var seatApplyList: [CommerceMicSeatApply] = [CommerceMicSeatApply]()
//    private var seatInvitationList: [ShowMicSeatInvitation] = [ShowMicSeatInvitation]()
    private var pkInvitationList: [CommercePKInvitation] = [CommercePKInvitation]()
    private var interactionList: [CommerceInteractionInfo] = [CommerceInteractionInfo]()
    ...
    private var pkCreatedInvitationMap: [String: CommercePKInvitation] = [String: CommercePKInvitation]()
...
//            agoraAssert("roomId == nil")
    ...
    private func cleanCache() {
        ...
        seatApplyList = [CommerceMicSeatApply]()
//        seatInvitationList = [ShowMicSeatInvitation]()
        pkInvitationList = [CommercePKInvitation]()
        interactionList = [CommerceInteractionInfo]()
        pkCreatedInvitationMap = [String: CommercePKInvitation]()
...
//            agoraAssert("leaveRoom channelName = nil")
    ...
    func getAllMicSeatApplyList(completion: @escaping (NSError?, [CommerceMicSeatApply]?) -> Void) {
    ...
    @objc func createMicSeatApply(completion: @escaping (NSError?) -> Void) {
        let apply = CommerceMicSeatApply()
...
//            agoraAssert("cancel apply not found")
    ...
    func acceptMicSeatApply(apply: CommerceMicSeatApply, completion: @escaping (NSError?) -> Void) {
        ...
        let interaction = CommerceInteractionInfo()
...
//        interaction.ownerMuteAudio = self.userMuteLocalAudio
    ...
    func rejectMicSeatApply(apply: CommerceMicSeatApply, completion: @escaping (NSError?) -> Void) {
        apply.status = .rejected
        _updateMicSeatApply(apply: apply, completion: completion)
    }
    
    func getAllMicSeatInvitationList(completion: @escaping (NSError?, [CommerceMicSeatInvitation]?) -> Void) {
//        _getAllMicSeatInvitationList(completion: completion)
        _getUserList(roomId: roomId, finished: completion)
    }
    
    func createMicSeatInvitation(user: CommerceUser, completion: @escaping (NSError?) -> Void) {
        //check interaction maximum
        if self.interactionList.count > 0 {
            completion(CommerceError.seatInteractionMaximumReach.toNSError())
            return
        }
...
//        _updateMicSeatInvitation(invitation: invitation, completion: completion)
    ...
    func getAllPKUserList(completion: @escaping (NSError?, [CommercePKUserInfo]?) -> Void) {
    ...
    func getAllPKInvitationList(completion: @escaping (NSError?, [CommercePKInvitation]?) -> Void) {
        _getAllPKInvitationList(room: nil, completion: completion)
    }
    
    func getCurrentApplyUser(roomId: String?, completion: @escaping (CommerceRoomListModel?) -> Void) {
        _getCurrentApplyUser(roomId: roomId, completion: completion)
    }
    
    func createPKInvitation(room: CommerceRoomListModel,
                            completion: @escaping (NSError?) -> Void) {
        //if robot room, reject
        if room.roomId.count > 6 {
            completion(CommerceError.userCannotAccept.toNSError())
            return
        }
        
        //check interaction maximum
        if self.interactionList.count > 0 {
            completion(CommerceError.pkInteractionMaximumReach.toNSError())
            return
        }
        ...
        _getAllPKInvitationList(room: room) {[weak self] error, invitationList in
            ...
            guard let invitation = invitationList.filter({ $0.fromRoomId == self.roomId }).first else {
                ...
                let _invitation = CommercePKInvitation()
...
//            interaction.ownerMuteAudio = self.userMuteLocalAudio
    ...
    func getAllInterationList(completion: @escaping (NSError?, [CommerceInteractionInfo]?) -> Void) {
//        interactionPaddingIdsSet.removeAll()
        _getAllInteractionList(completion: completion)
    }
    
    func stopInteraction(interaction: CommerceInteractionInfo, completion: @escaping (NSError?) -> Void) {
...
extension CommerceSyncManagerServiceImp {
    ...
    private func _updateRoomInteractionStatus(status: CommerceInteractionStatus) {
...
//        userListCountDidChanged?(UInt(count))
    ...
    private func _updateInteractionStatus(with status: CommerceInteractionStatus) {
        guard let channelName = roomId,
              let roomInfo = roomList?.filter({ $0.roomId == self.getRoomId() }).first,
              roomInfo.ownerId == VLUserCenter.user.id
        else {
...
//MARK: Seat Apply
extension CommerceSyncManagerServiceImp {
    private func _getAllMicSeatApplyList(completion: @escaping (NSError?, [CommerceMicSeatApply]?) -> Void) {
        let channelName = getRoomId()
        ...
        SyncUtil
            ...
            .get(success: { [weak self] list in
                ...
                let seatApplyList = list.compactMap({ CommerceMicSeatApply.yy_model(withJSON: $0.toJson()!)! })
        ...
        SyncUtil
            ...
            .subscribe(key: SYNC_MANAGER_SEAT_APPLY_COLLECTION,
                       onCreated: { _ in
                           ...
                           guard let self = self,
                                 let jsonStr = object.toJson(),
                                 let model = CommerceMicSeatApply.yy_model(withJSON: jsonStr) else { return }
                           defer {
                           ...
                           guard let self = self else {return}
                           var model: CommerceMicSeatApply? = nil
    ...
    private func _addMicSeatApply(apply: CommerceMicSeatApply, completion: @escaping (NSError?) -> Void) {
        let channelName = getRoomId()
    ...
    private func _removeMicSeatApply(apply: CommerceMicSeatApply, completion: @escaping (NSError?) -> Void) {
        let channelName = getRoomId()
    ...
    private func _updateMicSeatApply(apply: CommerceMicSeatApply, completion: @escaping (NSError?) -> Void) {
        let channelName = getRoomId()
...
//MARK: PK Invitation
extension CommerceSyncManagerServiceImp {
    fileprivate func _getAllPKInvitationList(room: CommerceRoomListModel?,
                                         completion: @escaping (NSError?, [CommercePKInvitation]?) -> Void) {
            ...
            SyncUtilsWrapper.joinSceneByQueue(id: channelName,
                ...
                SyncUtil
                    ...
                    .get(success: {  list in
                        ...
                        let pkInvitationList = list.compactMap({ CommercePKInvitation.yy_model(withJSON: $0.toJson()!)! })
    ...
    private func _subscribePKInvitationChanged() {
        ...
        _subscribePKInvitationChanged(channelName: channelName) { [weak self] (status, invitation) in
            ...
            switch status {
            case .deleted:
                var model: CommercePKInvitation? = nil
    ...
    private func _subscribePKInvitationChanged(channelName:String,
                                               subscribeClosure: @escaping (CommerceSubscribeStatus, CommercePKInvitation) -> Void) {
        ...
        SyncUtil
            ...
            .subscribe(key: SYNC_MANAGER_PK_INVITATION_COLLECTION,
                ...
                guard let jsonStr = object.toJson(),
                      let model = CommercePKInvitation.yy_model(withJSON: jsonStr) else {
                    return
                }
                ...
                guard channelName == self.roomId else {
                    if let model = CommercePKInvitation.yy_model(withJSON: object.toJson() ?? "") {
                ...
                guard let model = CommercePKInvitation.yy_model(withJSON: object.toJson() ?? "") else { return }
                subscribeClosure(.deleted, model)
            }, onSubscribed: {
            }, fail: { error in
    ...
    private func _addPKInvitation(invitation: CommercePKInvitation, completion: @escaping (NSError?) -> Void) {
    ...
    private func _removePKInvitation(invitation: CommercePKInvitation, completion: @escaping (NSError?) -> Void) {
        let channelName = invitation.roomId
        if channelName.isEmpty {return}
    ...
    private func _updatePKInvitation(invitation: CommercePKInvitation, completion: @escaping (NSError?) -> Void) {
        let channelName = invitation.roomId
        if channelName.isEmpty {return}
    ...
    private func _recvPKRejected(invitation: CommercePKInvitation) {
...
//        }
    ...
    private func _recvPKAccepted(invitation: CommercePKInvitation) {
        guard roomId == invitation.fromRoomId else { return }
...
//        interaction.ownerMuteAudio = self.userMuteLocalAudio
        ...
        self.pkCreatedInvitationMap.forEach { (key: String, value: CommercePKInvitation) in
    ...
    private func _recvPKFinish(invitation: CommercePKInvitation) {
        guard roomId == invitation.fromRoomId else { return }
        let pkRoomId = invitation.roomId
        _unsubscribePKInvitationChanged(roomId: pkRoomId)
//        SyncUtil.scene(id: pkRoomId)?.deleteScenes()
    ...
    func _getCurrentApplyUser(roomId: String?, completion: @escaping (CommerceRoomListModel?) -> Void) {
        guard let channelName = roomId else {
    ...
    func _handleCreatePkInvitationRespone(invitation: CommercePKInvitation, status: CommerceSubscribeStatus) {
        if status == .created {
            self.pkCreatedInvitationMap[invitation.roomId] = invitation
        }
...
//MARK: Interaction
extension CommerceSyncManagerServiceImp {
    private func _getAllInteractionList(completion: @escaping (NSError?, [CommerceInteractionInfo]?) -> Void) {
        guard let channelName = roomId else {
            agoraPrint("channelName = nil")
            return
        }
        ...
        SyncUtil
            ...
            .get(success: { [weak self] list in
                ...
                let interactionList = list.compactMap({ CommerceInteractionInfo.yy_model(withJSON: $0.toJson()!)! })
    ...
    private func _subscribeInteractionChanged() {
        ...
        SyncUtil
            ...
            .subscribe(key: SYNC_MANAGER_INTERACTION_COLLECTION,
                ...
                guard let self = self,
                      let jsonStr = object.toJson(),
                      let model = CommerceInteractionInfo.yy_model(withJSON: jsonStr) else {
                    return
                }
...
//                    self.subscribeDelegate?.onInteractionBegan(interaction: model)
                ...
                guard let self = self else {return}
                var model: CommerceInteractionInfo? = nil
                if let index = self.interactionList.firstIndex(where: { object.getId() == $0.objectId }) {
                    model = self.interactionList[index]
                    self.interactionList.remove(at: index)
                }
                guard let _invitation = model ?? CommerceInteractionInfo.yy_model(withJSON: object.toJson() ?? "") else {
    ...
    private func _addInteraction(interaction: CommerceInteractionInfo, completion: @escaping (NSError?) -> Void) {
        guard let channelName = roomId else {
//            assert(false, "channelName = nil")
    ...
    private func _removeInteraction(invitation: CommercePKInvitation, completion: @escaping (NSError?) -> Void) {
        //TODO:
    ...
    private func _removeInteraction(interaction: CommerceInteractionInfo, completion: @escaping (NSError?) -> Void) {
        guard let channelName = roomId else {
    ...
    private func _updateInteraction(interaction: CommerceInteractionInfo, completion: @escaping (NSError?) -> Void) {
        guard let channelName = roomId else {
