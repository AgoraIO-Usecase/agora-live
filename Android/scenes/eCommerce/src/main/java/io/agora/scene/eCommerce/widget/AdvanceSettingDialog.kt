...
import io.agora.scene.eCommerce.VideoSetting.toIndex
import io.agora.scene.eCommerce.databinding.CommerceSettingAdvanceAudioBinding
import io.agora.scene.eCommerce.databinding.CommerceSettingAdvanceDialogBinding
import io.agora.scene.eCommerce.databinding.CommerceSettingAdvanceItemSeekbarBinding
import io.agora.scene.eCommerce.databinding.CommerceSettingAdvanceItemSelectorBinding
import io.agora.scene.eCommerce.databinding.CommerceSettingAdvanceItemSwitchBinding
import io.agora.scene.eCommerce.databinding.CommerceSettingAdvanceItemSwitchSeekbarBinding
import io.agora.scene.eCommerce.databinding.CommerceSettingAdvanceVideoBinding
import io.agora.scene.eCommerce.utils.toInt
...
class AdvanceSettingDialog constructor(context: Context, val rtcConnection: RtcConnection) :
    ...
    private val mBinding by lazy {
        CommerceSettingAdvanceDialogBinding.inflate(
            LayoutInflater.from(
                context
            )
        )
    ...
    private val tagPageList = arrayListOf(
        TabPage(
            ...
            R.string.commerce_setting_advance_video_setting,
        ),
        TabPage(
            VIEW_TYPE_AUDIO_SETTING,
            R.string.commerce_setting_advance_audio_setting,
        )
    )
    ...
    private val presetChangeTipDialog by lazy {
        AlertDialog.Builder(context, R.style.commerce_alert_dialog).apply {
            setTitle(R.string.commerce_tip)
            setMessage(R.string.commerce_setting_advance_preset_mode)
            setPositiveButton(R.string.commerce_setting_confirm) { dialog, _ ->
                presetMode = false
                dialog.dismiss()
            }
            setNegativeButton(R.string.commerce_setting_cancel) { dialog, _ ->
                dialog.dismiss()
            }
        }.create()
    }
        ...
        mBinding.viewPager2.adapter = object : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
            override fun onCreateViewHolder(
                ...
                VIEW_TYPE_VIDEO_SETTING -> BindingViewHolder(
                    CommerceSettingAdvanceVideoBinding.inflate(
                        LayoutInflater.from(parent.context), parent, false
                    )
                )

                VIEW_TYPE_AUDIO_SETTING -> BindingViewHolder(
                    CommerceSettingAdvanceAudioBinding.inflate(
                        LayoutInflater.from(parent.context), parent, false
                    )
                )
            ...
            override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
                when (getItemViewType(position)) {
                    VIEW_TYPE_VIDEO_SETTING -> (holder as? BindingViewHolder<*>)?.binding?.let {
                        updateVideoSettingView(it as CommerceSettingAdvanceVideoBinding)
                    }

                    VIEW_TYPE_AUDIO_SETTING -> (holder as? BindingViewHolder<*>)?.binding?.let {
                        updateAudioSettingView(it as CommerceSettingAdvanceAudioBinding)
                    }
     ...
     * @param binding
     */
    private fun updateAudioSettingView(binding: CommerceSettingAdvanceAudioBinding) {
        setupSwitchItem(
            ITEM_ID_SWITCH_EAR_BACK,
            binding.earBack,
            R.string.commerce_setting_advance_ear_back,
            R.string.commerce_setting_advance_ear_back_tip
        )
        setupSeekbarItem(
            ITEM_ID_SEEKBAR_VOCAL_VOLUME,
            binding.vocalVolume,
            R.string.commerce_setting_advance_vocal_volume,
            "%d",
            0, 100
        )
        setupSeekbarItem(
            ITEM_ID_SEEKBAR_MUSIC_VOLUME,
            binding.musicVolume,
            R.string.commerce_setting_advance_music_volume,
            "%d",
            0, 100
        )
    }
    ...
    private fun updateVideoSettingView(binding: CommerceSettingAdvanceVideoBinding) {
        setupSwitchItem(
            ...
            R.string.commerce_setting_advance_color_enhance_tip
        )
        setupSwitchItem(
            ITEM_ID_SWITCH_DARK_ENHANCE,
            binding.darkEnhance,
            R.string.commerce_setting_advance_dark_enhance,
            R.string.commerce_setting_advance_dark_enhance_tip
        )
        setupSwitchItem(
            ITEM_ID_SWITCH_VIDEO_NOISE_REDUCE,
        ...
        setupSelectorItem(
            ITEM_ID_SELECTOR_RESOLUTION,
            binding.resolution,
            R.string.commerce_setting_advance_encode_resolution,
        ...
        setupSwitchAndSeekbarItem(
            ...
            binding.bitrate,
            R.string.commerce_setting_advance_bitrate,
            R.string.commerce_setting_advance_bitrate_tips,
            "%d kbps",
            200, 4000
        )
    }
    ...
    private fun setupSwitchItem(
        ...
        if (itemShowTextMap[itemId] == true) {
            ...
            binding.tvSwitch.text =
                context.getString(if (isChecked) R.string.commerce_setting_opened else R.string.commerce_setting_closed)
        } else {
    ...
    private fun setupSelectorItem(
        itemId: Int,
        binding: CommerceSettingAdvanceItemSelectorBinding,
        @StringRes title: Int,
        @StringRes tip: Int = -1,
    ...
    private fun setupSeekbarItem(
        itemId: Int,
        binding: CommerceSettingAdvanceItemSeekbarBinding,
        @StringRes title: Int,
        valueFormat: String,
        fromValue: Int,
        toValue: Int
    ...
    private fun setupSwitchAndSeekbarItem(
        itemIdSwitch: Int,
        itemIdSeekbar: Int,
        binding: CommerceSettingAdvanceItemSwitchSeekbarBinding,
        @StringRes title: Int,
        @StringRes tip: Int,
        valueFormat: String,
        fromValue: Int,
