...
import io.agora.scene.eCommerce.databinding.CommerceSettingPresetDialogBinding
...
class PresetDialog constructor(context: Context, deviceScore: Int, rtcConnection: RtcConnection) : BottomFullDialog(context) {
    ...
    private val mBinding by lazy {
        CommerceSettingPresetDialogBinding.inflate(
            LayoutInflater.from(
                context
            )
        )
    ...
    init {
        ...
        val deviceLevelStr = if (deviceScore >= 90) {
            context.getString(R.string.commerce_setting_preset_device_high)
        } else if (deviceScore >= 75) {
            context.getString(R.string.commerce_setting_preset_device_medium)
        } else {
            mBinding.tvBeautyTips.isVisible = true
            context.getString(R.string.commerce_setting_preset_device_low)
        }
        mBinding.tvDeviceScore.text = context.getString(R.string.commerce_video_device_result, deviceLevelStr, deviceScore)
    }
    ...
    private fun onPresetNetworkModeSelected(networkLevel: Int, broadcastStrategyLevel: Int, device: Int?){
        if (networkLevel < 0 || broadcastStrategyLevel < 0) {
            // Default to optimal network settings if none are selected.
            return
        }
